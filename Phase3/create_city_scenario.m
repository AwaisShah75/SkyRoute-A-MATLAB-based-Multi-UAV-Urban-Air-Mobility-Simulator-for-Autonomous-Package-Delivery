...
%   map3D    - occupancyMap3D object for collision checking
    ...
    % 1. Initialize the UAV Scenario with a higher frame limit (fits 10 drones + obstacles)
    scenario = uavScenario('UpdateRate', 10, 'ReferenceLocation', [0 0 0], 'MaxNumFrames', 50);
    ...
    for i = 1:numBuildings
        ...
        % Visual Polish: Create a 3-tier building with a spire (premium look)
        % Tier 1: Base (60% height, dark gray)
        addMesh(scenario, 'polygon', {vertices, [0 h*0.6]}, [0.35 0.35 0.35]);
        
        % Tier 2: Middle (25% height, scaled down to 80%, medium gray)
        midVertices = (vertices - pos) * 0.8 + pos;
        addMesh(scenario, 'polygon', {midVertices, [h*0.6 h*0.85]}, [0.5 0.5 0.5]);
        
        % Tier 3: Top (15% height, scaled down to 55%, light gray)
        topVertices = (vertices - pos) * 0.55 + pos;
        addMesh(scenario, 'polygon', {topVertices, [h*0.85 h]}, [0.65 0.65 0.65]);
        
        % Tier 4: Red spire/antenna at the very top (beacon look)
        spireVertices = (vertices - pos) * 0.08 + pos;
        addMesh(scenario, 'polygon', {spireVertices, [h h+6]}, [0.9 0.1 0.1]);
